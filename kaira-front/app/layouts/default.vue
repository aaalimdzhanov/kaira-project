<template>
    <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
        <defs>
            ...
            <symbol xmlns="http://www.w3.org/2000/svg" id="telegram" viewBox="0 0 24 24">
                <path fill="currentColor"
                    d="M9.993 15.067L9.81 19.14c.382 0 .547-.164.745-.36l1.787-1.71 3.706 2.717c.678.374 1.16.177 1.334-.627l2.416-11.354.001-.002c.215-.998-.362-1.39-1.019-1.149L3.682 10.29c-.977.38-.962.927-.166 1.174l3.912 1.22 9.074-5.724c.427-.287.814-.128.495.158l-7.004 6.95Z" />
            </symbol>
        ...
        <div class="offcanvas-body">
            <div class="order-md-last">
                <h4 class="d-flex justify-content-between align-items-center mb-3">
                    <span class="text-primary">{{ t('burger.your_cart') }}</span>
                ...
                <ul class="list-group mb-3">
                    ...
                    <li v-for="item in cart.items" :key="item.id"
                        ...
                        <div>
                            <h6 class="my-0">{{ item.name || 'Без названия' }}</h6>
                            <small class="text-body-secondary">
                                Цвет: {{ item.color?.title || '-' }}, Размер: {{ item.size?.title || '-' }}
                            </small>
                        </div>
                        <span class="text-body-secondary">{{ formatPrice(item.price) }}</span>
                    </li>
                    <!-- Общая сумма -->
                    <li class="list-group-item d-flex justify-content-between">
                        <span>{{ t('burger.total') }} (UZS)</span>
                        <strong>{{ formatPrice(totalPrice) }}</strong>
                    </li>
                </ul>
                <div class="d-grid gap-2">
                    <button class="w-100 btn btn-primary btn-lg" type="submit"  @click="cartClear">
                        {{ t('burger.clear') }}
                    </button>
                    <NuxtLink class="w-100 btn btn-primary btn-lg" to="/cart" active-class="active"
                        exact-active-class="active">
                        {{ t('burger.continue_checkout') }}
                    </NuxtLink>
                </div>
            </div>
        </div>
                    ...
                    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar"
                        ...
                        <div class="offcanvas-body">
                            <ul class="navbar-nav justify-content-end flex-grow-1 gap-1 gap-md-5 pe-3">
                                <li class="nav-item">
                                    <NuxtLink class="nav-link" to="/" active-class="active" exact-active-class="active">
                                        {{ t('menu.home') }}</NuxtLink>
                                </li>
                                <li class="nav-item">
                                    <NuxtLink class="nav-link" to="/clothing" active-class="active"
                                        exact-active-class="active">{{ t('menu.shop.clothing') }}</NuxtLink>
                                </li>
                                <li class="nav-item">
                                    <NuxtLink class="nav-link" to="/scarves" active-class="active"
                                        exact-active-class="active">{{ t('menu.shop.scarves') }}</NuxtLink>
                                </li>
                                <li class="nav-item">
                                    <NuxtLink class="nav-link" to="/accessories" active-class="active"
                                        exact-active-class="active">{{ t('menu.shop.accessories') }}
                                    </NuxtLink>
                                </li>
                                <li class="nav-item">
                                    <NuxtLink class="nav-link" to="/contact" active-class="active"
                                        exact-active-class="active">{{ t('menu.contact') }}</NuxtLink>
                                </li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <ul class="list-unstyled d-flex m-0">
                        <li class="d-none d-lg-block">
                            <div class="mx-2 d-flex gap-2">
                                <button @click="switchLang('en')" class="btn btn-link p-0">UZ</button>
                                <button @click="switchLang('ru')" class="btn btn-link p-0">RU</button>
                            </div>
                        </li>
                        <li class="d-none d-lg-block">
                            <NuxtLink to="/cart" class="text-uppercase mx-3" data-bs-toggle="offcanvas"
                            ...
                            <a href="#search" class="search-button">
                                <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
                                    <symbol id="search" viewBox="0 0 24 24">
                                        <path
                                            d="M21.53 20.47l-5.64-5.64A7.92 7.92 0 0016 9a8 8 0 10-8 8 7.92 7.92 0 005.83-2.11l5.64 5.64a.75.75 0 101.06-1.06zM4.5 9a6.5 6.5 0 1113 0 6.5 6.5 0 01-13 0z">
                                        </path>
                                    </symbol>
                                </svg>
                            </a>
    ...
    <section class="instagram position-relative">
        <div class="d-flex justify-content-center w-100 position-absolute bottom-0 z-1">
            <a href="https://www.instagram.com/templatesjungle/" class="btn btn-dark px-5">{{ t('menu.follow_instagram')
                }}</a>
        </div>
        <div class="row g-0">
            ...
            <div class="row d-flex flex-wrap justify-content-between py-5">
                ...
                <div class="col-md-3 col-sm-6">
                    <div class="footer-menu footer-menu-002">
                        <h5 class="widget-title text-uppercase mb-4">{{ t('menu.quick_links') }}</h5>
                        <ul class="menu-list list-unstyled text-uppercase border-animation-left fs-6">
                            <li class="menu-item">
                                <a href="index.html" class="item-anchor">{{ t('menu.home') }}</a>
                            </li>
                            <li class="menu-item">
                                <a href="shop-four-column-wide.html" class="item-anchor">{{ t('menu.about') }}</a>
                            </li>
                            <li class="menu-item">
                                <a href="#" class="item-anchor">{{ t('menu.contact') }}</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="col-md-3 col-sm-6">
                    <div class="footer-menu footer-menu-003">
                        <h5 class="widget-title text-uppercase mb-4">{{ t('menu.help_info') }}</h5>
                        <ul class="menu-list list-unstyled text-uppercase border-animation-left fs-6">
                            <li class="menu-item">
                ...
                <div class="col-md-3 col-sm-6">
                    <div class="footer-menu footer-menu-004 border-animation-left">
                        <h5 class="widget-title text-uppercase mb-4">{{ t('menu.contact_us') }}</h5>
                        <p>{{ t('menu.questions') }}<a href="mailto:contact@yourcompany.com"
                                class="item-anchor">kaira.shop@gmail.com</a></p>
                        <p>{{ t('menu.support_call') }}<a href="tel:+43720115278" class="item-anchor">+43 720
                                115 278</a>
                        </p>
                    </div>
                </div>
