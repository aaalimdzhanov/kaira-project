    ...
    <section class="pt-3 border-bottom">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                 <h1 class="text-uppercase fs-3">{{ t('menu.shop.accessories') }}</h1>
                <nav class="breadcrumb">
                    <NuxtLink to="/" class="breadcrumb-item">{{ t('menu.home') }}</NuxtLink>
                    <span class="breadcrumb-item active" aria-current="page">{{ t('menu.shop.accessories') }}</span>
                </nav>
            </div>
        </div>
    </section>
...
const { data, pending, error, refresh } = await useAsyncData(
    'accessories',
    () => $fetch(`/api/accessories?page=${currentPage.value}&lang=${locale.value}`)
)
