    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sizes.update", [$size->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $size->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category_ids') ? 'is-invalid' : '' }}" name="category_ids_id" id="category_ids_id" required>
                    @foreach($category_ids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_ids_id') ? old('category_ids_id') : $size->category_ids->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
